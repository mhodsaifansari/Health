...
function HospitalRegistration(props) {
 ...
 axios.post(`${URL}/hospital/register`, requestObject)
 .then((res)=>{
    localStorage.setItem('type',"hospital");
    localStorage.setItem('eid',res.data["E-id"])
    axios.post(`${URL}/hospital/login-otp`, { "eid":res.data["E-id"] })
    .then((data)=>{
        ...
        <div className={classes["signup-inputs-container"]}>
          <div className={classes["custom-input-container"]}>
            ...
            <Input
              ...
              onChange={(val) =>
                setUserData((state) => {
                  return { ...state, name: val.target.value };
                })
              }
